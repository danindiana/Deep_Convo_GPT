...
def main():
    ...
    ips = [get_ip_address(iface) for iface in selected_interfaces]
    
    if None in ips:
        logging.error("Could not retrieve IP addresses for one or more selected interfaces.")
        return
    
    logging.info(f"Testing data transfer between {selected_interfaces[0]} ({ips[0]}) and {selected_interfaces[1]} ({ips[1]})")
